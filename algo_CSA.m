...
Civicioglu, P., and E. Besdok. 2024. ''Colony-Based Search Algorithm for numerical optimization.'' \emph{Applied Soft Computing} 151: 111162.

%}

function [out,p0,fitp0,say]  =  algo_CSA(fnc , mydata , N , D , low , up ,Epk)
out.history=nan*ones(Epk,1);
say=0;
% Search Space limits
if numel(low)<D, low = low*ones(1,D); up = up*ones(1,D); end
% Initialization
T = 1  ;                      % Pattern matrix expanding factor or Colony (i.e., swarm)  size
initindex = 1:N;
moment = 0;
p0 = nan*rand(T*N,D);     % Pattern matrix or Colony
...
% if isfield(mydata,'say'), say=mydata.say;  end
% if isfield(mydata,'gbest'), p0(1,:)=mydata.gbest; end
% if isfield(mydata,'initpop'), initpop=mydata.initpop; end
% p0(1:size(initpop,1),:)=initpop;

fitp0  =  feval(fnc,p0,mydata);
say=say+size(p0,1);

out.history(1)=min(fitp0);
...
for epoch  =  1:Epk
    ...
    % yeni-eklenen
    jind=randperm(N,4);
    dv1=px(jind(2),:)-px(jind(1),:);
    dv2=px(jind(4),:)-px(jind(3),:);
    w=rand;
    px(jind(1),:)=px(jind(1),:)+w*dv1+(1-w)*dv2;
    % ----------------
    ...
    out.p0=p0;
    out.history(epoch+1)=gmin;
    assignin('base','out',out)
    fprintf('%3.0f-> %5.16f  | fncsay: %8.0f \n',epoch,gmin,say)
